...
export default function BookPlayerPage() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.code === "ArrowRight" && book && currentChapterId) {
        ...
        if (idx < book.chapters.length - 1) { setCurrentChapterId(book.chapters[idx + 1].id); setIsPlaying(true); }
      }
      if (e.code === "ArrowLeft" && book && currentChapterId) {
        const idx = book.chapters.findIndex((c) => c.id === currentChapterId);
        if (idx > 0) { setCurrentChapterId(book.chapters[idx - 1].id); setIsPlaying(true); }
      }
    };
  ...
  const handleDownload = () => {
    ...
    const url = api.getDownloadUrl(book!.id);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${book!.title}.${selectedFormat}`;
    a.click();
    toast(`Downloading ${selectedFormat.toUpperCase()}...`, "success");
  };
    ...
    <div className="min-h-screen bg-[var(--surface-alt)]">
      <header className="bg-[var(--surface)] border-b border-[var(--border)] sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
              ...
              <div className="relative">
                ...
                {showFormatPicker && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-1 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-xl z-20 py-1 min-w-[140px]">
                      {FORMATS.map((f) => (
            ...
            {isDemo && <span className="text-[10px] font-medium text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded-full hidden sm:inline">Demo</span>}
          </div>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 sm:py-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm text-[var(--text-secondary)]">
          ...
          <div className="flex items-center gap-1.5"><Clock className="w-4 h-4" /><span>{Math.floor(totalDuration / 60)} min total</span></div>
          {completedCount > 0 && <><span className="hidden sm:inline text-[var(--border)]">|</span><span className="text-emerald-600 dark:text-emerald-400">{Math.round((completedCount / book.chapters.length) * 100)}% complete</span></>}
        </div>
        ...
        <div>
          ...
          {book.chapters.length > 0 ? (
            ...
            <div className="card p-12 text-center">
              ...
              <p className="text-[var(--text-secondary)] font-medium">No chapters available yet</p>
              <p className="text-sm text-[var(--text-muted)] mt-1">Conversion is in progress</p>
            </div>
          )}
        </div>
      </main>
